...
test.requestHooks(mockEnrollAuthenticatorPassword)('should load select authenticator list', async t => {
  ...
  await t.expect(selectFactorPage.getFactorLabelByIndex(5)).eql('Atko Custom On-prem');
  await t.expect(selectFactorPage.getFactorIconClassByIndex(5)).contains('mfa-onprem');
  await t.expect(selectFactorPage.getFactorSelectButtonByIndex(5)).eql('Set up');
  await t.expect(selectFactorPage.getFactorSelectButtonDataSeByIndex(5)).eql('del_oath-otp');
